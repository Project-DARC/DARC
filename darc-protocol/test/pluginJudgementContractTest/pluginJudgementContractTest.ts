...
describe.only("plugin judgement contract test", function () {
  it ("should pass all plugin judgement test in PluginJudgementTest.sol", async function () {

    const PluginJudgementTestFactory = await ethers.getContractFactory("PluginJudgementTest");
    ...
    expect(bIsSuccess).to.equal(true);
  });
