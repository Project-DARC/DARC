...
describe.only("multi class token voting test", function () {
  it ("should pass multi class token voting test", async function () {
    ...
    // make sure that current state is voting
    expect((await votingTestSingleTest.finiteState()).toString()).to.equal("2"); // should be FiniteState.VOTING
    expect((await votingTestSingleTest.getVotingItemsByIndex(1)).powerYes[0].toString()).to.equal("6"); // should be 100

    // target 1 vote, 13/31 willvoted
    await letAddressVote(target1, votingTestSingleTest.address);

    // make sure that current state is voting
    expect((await votingTestSingleTest.finiteState()).toString()).to.equal("2"); // should be FiniteState.VOTING
    expect((await votingTestSingleTest.getVotingItemsByIndex(1)).powerYes[0].toString()).to.equal("31"); // should be 300
